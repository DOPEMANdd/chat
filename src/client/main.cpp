            ...
            else
            {
                ...
                else
                {
                    ...
                    else // 登录成功
                    {
                        ...
                        // 登录成功，启动接收线程负责接受数据，只启用一次即可；
                        static int readthreadnumber = 0; // 注意静态变量的使用（很厉害）
...
void readTaskHandler(int clientfd)
{
    ...
    {
        char buffer[1024] = {0};
        int len = recv(clientfd, buffer, 1024, 0);  //loginout后必须quit，不然继续login的话，服务端返回的buffer会被该线程读取
        if (-1 == len || 0 == len)
        {
            close(clientfd);
            exit(-1);
...
void mainMenu(int clientfd)
{
    ...
    while (isMainMenuRunning)
    {
        ...
        {
            command = commandbuf.substr(0, idx); // 取命令
        }
...
void chat(int clientfd, string str)
{
    ...
    string message = str.substr(idx + 1, str.size() - idx); // 取子串第二个参数长度如果超过母串范围，自动截取
