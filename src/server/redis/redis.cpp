...
void Redis::observer_channel_message()
{
    ...
    while (REDIS_OK == redisGetReply(this->_subcribe_context, (void **)&reply)) //收到消息，其实是redis->server
    {
        // 订阅收到的消息是一个带三元素的数组
        if (reply != nullptr && reply->element[2] != nullptr && reply->element[2]->str != nullptr)
        {
            // 给业务层上报通道上发生的消息
            _notify_message_handler(atoi(reply->element[1]->str), reply->element[2]->str);  //server->client
        }
